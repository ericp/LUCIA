...
def generate_pdf_report(outdir, overall_accuracy, acc_by_object, acc_by_user, acc_by_cond, summary_df, notes_sample=None):
    ...
    story.append(Paragraph("<b>Accuracy by image condition (first rows)</b>", styles['Heading3']))
    ...
    if notes_sample is not None and len(notes_sample) > 0:
        story.append(Paragraph("<b>Sample of user notes</b>", styles['Heading2']))
    ...
    doc.build(story)
    print(f"PDF report saved in {pdf_path}")
...
def main(input_path, out_dir):
    ...
    print(" Analysis completed. Results in:", out_dir)

    # Generate PDF report using calculated variables
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", default="TestsResults.xlsx", help="Path to Excel file")
    parser.add_argument("--outdir", default="reports", help="Output folder")
